...
{
	gCurHierInfo++;
	if (gCurHierInfo > MAX_HIER)
	{
		gErrorCode = ERR_OUT_OF_MEMORY;
		return NULL;
	}
	return &gHierInfos[gCurHierInfo];
}
	...
	while (syn->token != 0)
	{
		if (syn->token == token)
		{
			if (syn->Syntax[0] != 0)
			{
				typ = gExprType;
				if (Expect(OP_BOPEN)) return 0;
				CompileSyntax(syn->Syntax);
				if (Expect(OP_BCLOSE)) return 0;
				gExprType = typ;
			}
			if (gErrorCode != 0) return 0;
			*gCodePtr++ = syn->op1;
			if (syn->op1 == CMD_API) *gCodePtr++ = syn->op2;
			PUSH_TYPE(syn->returntype);
			break;
		}
		syn++;
	}
...
U8 Expr4()
{
	...
	while (valid)
	{
		...
		switch(token)
		{
				case OP_BIGGER:			r = Expr2();		
										typ = ExtendTypes();
										*gCodePtr++ = CMD_CMP + typ;
										*gCodePtr++ = CMD_LOADF_LE;
										PUSH_TYPE(TYP_8);
										break;
				case OP_BIGGEREQUAL:	r = Expr2();		
										typ = ExtendTypes();
										*gCodePtr++ = CMD_CMP + typ;
										*gCodePtr++ = CMD_LOADF_LT;
										PUSH_TYPE(TYP_8);
										break;
				case OP_SMALLER:		r = Expr2();		
										typ = ExtendTypes();
										*gCodePtr++ = CMD_CMP + typ;	
										*gCodePtr++ = CMD_LOADF_GE;
										PUSH_TYPE(TYP_8);
										break;
				case OP_SMALLEREQUAL:	r = Expr2();		
										typ = ExtendTypes();
										*gCodePtr++ = CMD_CMP + typ;	
										*gCodePtr++ = CMD_LOADF_GT;
										PUSH_TYPE(TYP_8);
										break;
				case OP_EQUAL:			r = Expr2();		
										typ = ExtendTypes();
										*gCodePtr++ = CMD_CMP + typ;	
										*gCodePtr++ = CMD_LOADF_Z;
										PUSH_TYPE(TYP_8);
										break;
				case OP_UNEQUAL:		r = Expr2();		
										typ = ExtendTypes();
										*gCodePtr++ = CMD_CMP + typ;	
										*gCodePtr++ = CMD_LOADF_NZ;
										PUSH_TYPE(TYP_8);
										break;
...
/* Push the to value on the stack */ 
	...
	hier = &gHierInfos[gCurHierInfo++];
	hier->hierTyp = HIER_FOR;
	hier->hierVar = var-gVariables;

	*gCodePtr++ = CMD_JUMP;					/* Jump to the compare routine in the stack */
	hier->hierExit = (U16*)gCodePtr;		/* Remember where to jump                   */
	*((U16*)gCodePtr)++ = 0;		
	hier->hierAddress = gCodePtr-gCode;		/* Note the addr where code starts			*/
}
...
void CompileNext()
{
	...
	/* At this point, the code between for and next has been executed. We 
	   only have the maximum value on the stack. Now we must compare it with
	   the value stored in our variable. If it is still less, we jump to the beginning 
	   of the loop, if not, we jump to the exit
	*/

	--gCurHierInfo;

	hier = &gHierInfos[gCurHierInfo];
	var  = &gVariables[hier->hierVar];
...
U8 CompileCommandViaTable(U8 token)
{
	...
	if (tok == NULL)						return 0;
	if (tok->returntype != TYP_VOID )     return 0;
	InsertDebugLine();
	if (tok->syntax != NULL)
		CompileSyntax(tok->syntax);
	*gCodePtr++ = tok->op1;
...
//	itoa(token,gTempBuf,10);
	switch(token)
	{
		...
		case OP_NEXT:		CompileNext();							break;
...
void TestCompiler()
{
	...
	/* Initialize the memory for program structures */

	gCurHierInfo = 0;

	/* memset(gCode,0,CODESIZE); */
	gExprType = TYP_32;
	gCodePtr = gCode;
	gLastVar  = 0;
	*gSource = 0;
	memset(gSource,0,SOURCESIZE);
...
/*
		...
		ReportError();
		gSourcePtr = gSource;
		gCompilerShowListing = 0;
		gPass = 2;
		_print("Pass 2\n");
		Compile();
		ReportError();

		if (gErrorCode == 0 || gErrorCode == 10) 
		{
			strcpy(gPrintBuf,__nl);
			it_printtxt(&gWindow,gPrintBuf);

			*gCodePtr++ = CMD_HALT;
			*gCodePtr++ = CMD_HALT;
			strcpy(gPrintBuf,"OK Press 'a' to view assembly code");
			it_printtxt(&gWindow,gPrintBuf);
			key = GetKey();
			if (key == 'a') {
				FillScreen(WHITE);
				it_BasicSetup(&gWindow);
				Disass(gCode,gCodePtr,1);
				key = GetKey();
			} else
			{
				_print("\nWriting assembly output\n");
				Disass(gCode,gCodePtr,0);
				FillScreen(WHITE);
				it_BasicSetup(&gWindow);
			}
			ExecuteVM(gCode);
			_print("Execution terminated, press a key to exit\n");
		} else
			_print("Not executed due to error in basic app");
